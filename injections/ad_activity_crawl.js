...
function getAdActivityList(url, lastAd) {
  ...
  request.onload = function() {
    if (request.status >= 200 && request.status < 400) {
      // Success!
      let respData = JSON.parse(request.responseText.replace("for (;;);", ""));
      let adClickedData = respData[PAYLOAD][HISTORYCLICKED][HISTORYROW];
      let hasMoreItems = respData[PAYLOAD][HISTORYCLICKED][HASMORE];
      let lastItem = respData[PAYLOAD][HISTORYCLICKED][LASTITEMSERVED];
      ...
      setTimeout(function() {
        let adIds = Object.keys(adClickedData);
        for (adId of adIds) {
...
function getAdActivityData(url) {
  ...
  request.onload = function() {
    if (request.status >= 200 && request.status < 400) {
      // Success!
      let respData = JSON.parse(request.responseText.replace("for (;;);", ""));
      let adContents = respData[PAYLOAD];
      let data = { data: adContents, type: "adActivityData" };
      window.postMessage(data, "*");
    } else {
      // We reached our target server, but it returned an error
    }
