...
function isOldExplanation(response) {
    let expStart = getIndexFromList(response,ABOUT_THIS_FACEBOOK_AD);
    let expEnd = getIndexFromList(response,MANAGE_YOUR_AD_PREFERENCES);
...
function getResourcesFromExplanationNewInterface(sr_revision_id, response) {
    let jsResources = []
    let html_page = response;
...
function getDocIdFromWaistResource(jsResource, module) {
    ...
    for (let i=0;i<resourceLines.length;i++) {
        if (resourceLines[i].indexOf(module+'",[')!==-1) {
            ...
            if(!matchs)
                matchs = resourceLines[i].match('e.exports="([0-9]*)"')
            if(!matchs)
                continue
            if (matchs.length >= 2) {
                return matchs[1]
    ...
    debugLog("Doc id was not found:"+ jsResource);
    return undefined
}
...
function split_into_normal_and_data_urls(urls) {
    let url_resources = []
    let data_url_resources = []

    for (let i =0; i<urls.length; i++) {
        if (isValidUrl(urls[i])) {
            url_resources.push(urls[i])
        }
        else {
            data_url_resources.push(urls[i])
        }
    }

    return [url_resources, data_url_resources]
}



function getDocIdFromDataUrlResources(urls, callback, module) {

    for (let i=0; i<urls.length; i++) {

        let data_from_url = atob(urls[i].split(',')[1]);

        if (data_from_url.indexOf(module)!==-1) {
            let docId = captureErrorOverload(getDocIdFromWaistResource,[data_from_url, module],undefined);
                if ((docId)) {
                    return docId
                }
        }
...
function getDocIdFromMenuResourcesNewInterface(objId,serialized_frtp_identifiers,story_debug_info,should_get_demographics, callback) {
    ...
    xmlhttp.onload = function(e) {
        ...
        let jsResources= getResourcesFromExplanationNewInterface(sr_revision_id, xmlhttp.responseText);

        [url_resources, data_url_resources] = split_into_normal_and_data_urls(jsResources)


        let module = "CometAdPrefsWAISTDialogRootQuery_facebookRelayOperation"
        let doc_id = captureErrorOverload(getDocIdFromDataUrlResources,[data_url_resources,callback, module],undefined);

        if (doc_id) {
            callback(doc_id)
        }
        else {
            debugLog("Not found in dataurls");
...
function getGraphQLExplanations(userId,adId,explanationUrl,dbRecordId,timestamp,graphQLAsyncParams,clientToken,docId,getNewDocId,newInterface,adType,objId,serialized_frtp_identifiers,story_debug_info) {
    ...
    xmlhttp.onload = function(e) {
        if (xmlhttp.readyState === 4 && xmlhttp.status === 200){
            let containsTargetingData = captureErrorOverload(containsTargetingDataGraphQL,[xmlhttp.response],undefined);
    ...
    let allParams = getGraphQLPostParameters(adId,clientToken,graphQLAsyncParams,docId);
...
function getGraphQLPostParameters(adId,clientToken,asyncParams,docId) {
    let params_extra = {'av':asyncParams.__user,'fb_api_caller_class':'RelayModern','fb_api_req_friendly_name':'AdsPrefWAISTDialogQuery',
