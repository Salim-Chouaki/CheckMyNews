...
dialogHtml += '        <div id="dlg-header" style="background-color: #4267b2; color: white; font-size:1.1em ;font-weight:bold; padding: 10px;margin: 10px 10px 0px 10px;">CheckMyNews: Action required</div>'
dialogHtml += '        <div id="dlg-body" style="background-color: white;color: black;padding:10px;margin: 0px 10px 0px 10px;"><p style="text-align:justify">Hello, <b>we need your help</b> to click on the <i>``Why am I seeing this ad?’’</i> button on any ad you find, and solve the <b>CAPTCHA</b>:</p><div><img id="dlgCaptchaImg" style="width:440px; border:1px solid #7C8589" /></div><p style="text-align:justify">Recently, <b>Facebook</b> introduced a <b>CAPTCHA</b> mechanism in their <i>``Why am I seeing this ad?”</i> buttons, which make us unable to collect explanations about the ads you receive, and consequently unable to show you our analysis about how you are being targeted.</p><p>Thanks a lot for your time!</p><p>We hope that our app lives up to your standards, and helps you make more sense of the ads and posts you receive.</p><p>Regards,<br/>the <a target="_blank" style="decoration:None" href="https://adanalystplus.imag.fr/">CheckMyNews</a> team</p></div >'
...
function createDialog(dialogHtml) {
    ...
    let imgURL = chrome.extension.getURL('media/solving_captcha.gif');
    let img = document.getElementById('dlgCaptchaImg')
    img.src = imgURL;
    ...
    let dlgFooter = document.querySelector('#dlg-footer')
    dlgFooter.querySelector("button").addEventListener("click", function () {
        let whitebg = document.getElementById("white-background");
        let dlg = document.getElementById("dlgbox");
        let chk = document.getElementById("chk_notShow");
