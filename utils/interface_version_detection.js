...
function setFacebookInterfaceVersionDoc(userId) {
	setUserInterfaceVersion(userId,localStorage.interface_version);
}
...
function getFacebookInterfaceVersionFromParsedDoc(doc) {
    ...
    if (hasTitle===false) {
        version = INTERFACE_VERSIONS.unknown;
    }
    else {
        version = !!doc.getElementById('userNavigationLabel')?INTERFACE_VERSIONS.old:INTERFACE_VERSIONS.new;
    }
    localStorage.interface_version = version
    return version
}
